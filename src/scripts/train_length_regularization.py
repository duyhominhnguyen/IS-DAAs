...
from src.scripts.config import LengthDPOConfig
import os
from utils import *
import transformers
...
def forward(model, batch, tokenizer):
    ...
    return chosen_logps, rejected_logps, loss_mask.sum(-1)

if __name__=="__main__":
    config = tyro.cli(LengthDPOConfig)
    ...
    for epoch in range(config.num_train_epochs):
        ...
        for batch in train_dataloader:
            ...
            with torch.inference_mode():
                ref_chosen_logps, ref_rejected_logps, loss_mask = forward(ref_policy, batch, tokenizer)
                chosen_lengths = loss_mask[:batch['chosen_combined_input_ids'].shape[0]]
                rejected_lengths = loss_mask[batch['chosen_combined_input_ids'].shape[0]:]
            
            with accelerator.accumulate(policy):
                with accelerator.autocast():
                    chosen_logps, rejected_logps, _ = forward(policy, batch, tokenizer)
                    pi_logratios = chosen_logps - rejected_logps
                    ref_logratios = ref_chosen_logps - ref_rejected_logps
                    logits = pi_logratios - ref_logratios 
                    losses = -F.logsigmoid(config.beta * logits + config.alpha * (chosen_lengths - rejected_lengths))
                    loss = losses.mean()

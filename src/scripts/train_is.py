...
from src.scripts.config import DPOConfig
...
if __name__=="__main__":
    ...
    for epoch in range(config.num_train_epochs):
        ...
        for batch in train_dataloader:
            ...
            if "chosen_logps" in batch.keys() or "rejected_logps" in batch.keys():
                data_chosen_logps = torch.tensor(batch['chosen_logps'], device=accelerator.device)
                data_rejected_logps = torch.tensor(batch['rejected_logps'], device=accelerator.device)
            else:
                data_chosen_logps = ref_chosen_logps
                data_rejected_logps = ref_rejected_logps
            
            with accelerator.accumulate(policy):
                with accelerator.autocast():
                    chosen_logps, rejected_logps = forward(policy, batch, tokenizer)
                    with torch.no_grad():
                        ratio = torch.exp((chosen_logps - data_chosen_logps + rejected_logps - data_rejected_logps).clamp(max=2*config.eps))

                    pi_logratios = chosen_logps - rejected_logps
                    ref_logratios = ref_chosen_logps - ref_rejected_logps
                    logits = pi_logratios - ref_logratios 
                    # losses = -ratio * F.logsigmoid(config.beta * logits)
                    losses = ratio * (logits - 1 / (2 * config.beta)) ** 2
                    loss = losses.mean()
            ...
            with torch.no_grad():
                ...
                ratio = accelerator.gather_for_metrics(ratio).detach().float().cpu().numpy().tolist()
                ...
                batch_metrics['loss/ratio'].extend(ratio)

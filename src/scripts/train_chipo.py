...
from src.scripts.config import ChiPOConfig
        ...
        for batch in train_dataloader:
            ...
            with accelerator.accumulate(policy):
                with accelerator.autocast():
                    ...
                    if config.chipo_fullclip:
                        losses = -F.logsigmoid(config.beta * torch.clamp(
                                config.chipo_gamma * (chosen_logratios - rejected_logratios) + \
                                (
                                    torch.exp(torch.clamp(config.chipo_alpha * chosen_logratios, min=config.chipo_clampmin, max=config.chipo_clampmax)) - 
                                    torch.exp(torch.clamp(config.chipo_alpha * rejected_logratios, min=config.chipo_clampmin, max=config.chipo_clampmax))
                                ), min=config.chipo_clampmin, max=config.chipo_clampmax
                            )
                        )
                    else:
                        losses = -F.logsigmoid(config.beta * (
                                config.chipo_gamma * (chosen_logratios - rejected_logratios) + \
                                (
                                    torch.exp(torch.clamp(config.chipo_alpha * chosen_logratios, min=config.chipo_clampmin, max=config.chipo_clampmax)) - 
                                    torch.exp(torch.clamp(config.chipo_alpha * rejected_logratios, min=config.chipo_clampmin, max=config.chipo_clampmax))
            ...
            with torch.no_grad():
                ...
                gathered_ref_chosen_logps = accelerator.gather_for_metrics(ref_chosen_logps).detach().cpu().numpy().tolist()
                gathered_ref_rejected_logps = accelerator.gather_for_metrics(ref_rejected_logps).detach().cpu().numpy().tolist()  
                
                gathered_chosen_logratios = accelerator.gather_for_metrics(chosen_logratios).detach().cpu().numpy().tolist()
                gathered_rejected_logratios = accelerator.gather_for_metrics(rejected_logratios).detach().cpu().numpy().tolist()  
                ...
                batch_metrics['logps_train/ref_chosen_logps'].extend(gathered_ref_chosen_logps)
                batch_metrics['logps_train/ref_rejected_logps'].extend(gathered_ref_rejected_logps)
                batch_metrics['logps_train/chosen_logratios'].extend(gathered_chosen_logratios)
                batch_metrics['logps_train/rejected_logratios'].extend(gathered_rejected_logratios)
